...
merge(Panda.prototype, {
  ...
  makeRequest: function (options,cb) {
    // File upload
    if (form._streams.length > 0) {
      options['protocol'] = options.port === 80 ? 'http:' : 'https:'; // it's necessary for form-data
  ...
  queryOptions: function(verb, uri, params) {
    ...
    var path = '';
    var headers = {};
    var signedParams = this.signedParams(verb, uri, params);

    if (verb === 'POST' && params['file'] !== undefined) {  // File upload
      var paramsKeys = Object.keys(signedParams);

      paramsKeys.forEach(function (key) {
        form.append(key, signedParams[key]);
      });
      form.append('file', fs.createReadStream(signedParams['file']));

      path = request_uri;
    }
    else {
      var query = auth.hashToQuery(signedParams);
