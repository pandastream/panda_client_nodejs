...
merge(Panda.prototype, {
  ...
  makeRequest: function (options,cb) {
    var self = this;
    var request = this.httpClient().request(req_options, function(res) {
      ...
      res.addListener('end', function (chunk) {
        if(cb) {
          ...
          {
            //Repeat the request again if the first attempt failed to parse.
            self.makeRequest(options, cb);
          }
        }
      });
    });
